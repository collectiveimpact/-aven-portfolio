...
type AnswerValue = number | string | string[] | null | undefined;
const dim = "var(--f5-text-muted)";

export function AnswerForm({
  id, title, description, questions,
}: {
  id: string; title: string; description: string; questions: BuilderQuestion[];
}) {
  ...
  if (done)
    return (
      <div className="f5-card" style={{ textAlign: "center", padding: 40 }}>
        <div style={{ fontSize: 36, color: "var(--f5-teal)" }} aria-hidden>✓</div>
        <div style={{ fontWeight: 800, fontSize: 19, color: "var(--f5-text)", marginTop: 8 }}>Thank you</div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div className="f5-card" style={{ borderTop: "3px solid var(--f5-teal)" }}>
        <div style={{ fontSize: 20, fontWeight: 800, color: "var(--f5-text)" }}>{title}</div>
        {description && <div style={{ color: dim, marginTop: 6 }}>{description}</div>}
        <div style={{ color: dim, fontSize: 12, marginTop: 8 }}>
          {questions.length} question{questions.length === 1 ? "" : "s"} · your answers are confidential
        </div>
      </div>

      {questions.map((q, i) => (
        <div key={q.id} className="f5-card">
          <div style={{ fontWeight: 600, color: "var(--f5-text)" }}>
            {i + 1}. {q.text || "(untitled)"}
            {q.required !== false && <span style={{ color: "var(--f5-red, #ef4444)", marginLeft: 4 }}>*</span>}
          </div>
          <div style={{ marginTop: 10 }}>
            <Field q={q} value={answers[q.id]} set={set} toggleMulti={toggleMulti} />
          </div>
        </div>
      ))}

      {error && <div style={{ color: "var(--f5-red, #ef4444)", fontSize: 13 }}>{error}</div>}
      <button
        ...
        {Array.from({ length: 11 }, (_, n) => {
          ...
          return (
            <button
              ...
              onClick={() => set(q.id, n)}
              style={{
                width: 38, height: 38, borderRadius: 8, border: "1px solid var(--f5-border)",
                background: on ? "var(--f5-teal)" : "transparent",
                color: on ? "#001417" : "var(--f5-text)", fontWeight: on ? 700 : 400, cursor: "pointer",
              }}
            >
              {n}
            </button>
        ...
        return (
          <label
            ...
            style={{
              ...
              border: `1px solid ${on ? "var(--f5-teal)" : "var(--f5-border)"}`, borderRadius: 8,
              cursor: "pointer", color: "var(--f5-text)",
            }}
            onClick={() => (isMulti ? toggleMulti(q.id, o) : set(q.id, val))}
          >
            <span style={{ color: on ? "var(--f5-teal)" : "var(--f5-text-muted)" }}>
              {isMulti ? (on ? "☑" : "☐") : on ? "◉" : "○"}
            </span>{" "}
            {o}
          </label>
