...
2. Apply migrations **in order** `0001 → 0011` from `fuse5-hub/supabase/migrations/`.
   Either:
...
- [ ] **Demo-data fallbacks** — `src/lib/queries.ts` getters return canned demo
      rows when a table is **empty** (great for the demo, wrong for a real empty
      org — a new provider with 0 residents would show fake ones). Before go-live,
      gate the `if (!data?.length) return DEMO.x` returns behind a server
      `DEMO_FALLBACK` flag (default off in prod) so real empty states render.
      *~61 call sites — one focused pass; ask and I'll do it.*
